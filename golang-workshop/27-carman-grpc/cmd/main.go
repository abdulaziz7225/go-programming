package main

import (
	"log"
	"net"

	"golang.source-fellows.com/training/carman/v6/api/proto"
	myhttp "golang.source-fellows.com/training/carman/v6/internal/http"
	mygrpc "golang.source-fellows.com/training/carman/v6/internal/grpc"
	"golang.source-fellows.com/training/carman/v6/internal/memory"
	"google.golang.org/grpc"
)

func main() {
	// 1. Initialize a single shared database
	repo := &memory.CarRepository{}

	// 2. Start the gRPC Server in a background Goroutine
	go func() {
		listener, err := net.Listen("tcp", ":9090")
		if err != nil {
			log.Fatalf("Failed to listen on gRPC port: %v", err)
		}
		
		grpcServer := grpc.NewServer()
		
		// Register our handler, passing in the shared repo
		handler := &mygrpc.GrpcHandler{Repo: repo}
		proto.RegisterCarServiceServer(grpcServer, handler)

		log.Println("gRPC Server listening on :9090")
		if err := grpcServer.Serve(listener); err != nil {
			log.Fatalf("Failed to serve gRPC: %v", err)
		}
	}()

	// 3. Start the Gin REST Server on the main thread
	log.Println("REST Server listening on :8080")
	if err := myhttp.StartServer(repo); err != nil {
		log.Fatalf("Failed to start REST server: %v", err)
	}
}